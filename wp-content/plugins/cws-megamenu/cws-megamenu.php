...
if ( !class_exists( "CWS_Megamenu" ) ){
    class CWS_Megamenu {
        ...
        public function enqueue_scripts (){
            wp_register_script ( 'cws_megamenu_front',  plugin_dir_url( __FILE__ ) . 'assets/js/cws_megamenu_front.js', array(), '', 'footer' );
        ...
        public function start_el_filter ( $item_output, $item, $depth, $args ){
            ...
            if ( ! empty( $vc_shortcodes_custom_css ) ) {
                ...
                // Cws_shortcode_css()->enqueue_cws_css($vc_shortcodes_styles);
            }
            ...
            if ( ! empty( $vc_post_custom_css ) ) {
                ...
                // Cws_shortcode_css()->enqueue_cws_css($vc_post_styles);
            }
            /*** \for composer ***/
            ...
            if ( $match ){
                ...
                if ( !empty( $custom_url ) ){
                    ...
                    $item_text = substr_replace( $item_text, "<a class='cws_megamenu_item_title'>" . $title . "</a><i class='button_open'></i>", $link_matches[1], strlen( $link_matches[0] ) );
                }
            }
...
function show_megamenuItems_in_navMenus( $user_id ) {
    ...
    $hidden_meta_boxes_arr = isset($user_meta[$metaboxhidden_key]) ? $user_meta[$metaboxhidden_key] : array();
